...
source_func = '/afs/cbs.mpg.de/projects/mar004_lsd-lemon-preproc/probands/%s/preprocessed/lsd_resting/%s/'
...
for sub_old in subjects_lsd: 
    ...
    if not os.path.isfile(source_mask %(sub_old) + 'T1_brain_mask.nii.gz'):
        print "%s brain mask missing" % sub_old
    ...
    if not os.path.isfile(brain_mask_template % (sub_new, sub_new, sess)): 
        print "copying %s to %s brain mask, session is %s" % (sub_old, sub_new, sess)
        shutil.copyfile(source_mask %(sub_old) + 'T1_brain_mask.nii.gz', brain_mask_template % (sub_new, sub_new, sess))
